...
class AbileneSegLoader(Dataset):
    ...
    def __len__(self):
        if self.flag == "train":
            return (self.train.shape[0] - self.win_size) // self.step + 1 #win_size = 100 , step = 1
        elif self.flag == 'val':
    ...
    def __getitem__(self, index):
        ...
        if self.flag == "train":
            data = np.float32(self.train[index:index + self.win_size])
            labels = np.zeros((self.win_size, data.shape[1]), dtype=np.float32)

        elif self.flag == 'val':
            data = np.float32(self.val[index:index + self.win_size])
            labels = np.zeros((self.win_size, data.shape[1]), dtype=np.float32)
